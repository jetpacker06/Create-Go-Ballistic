...
public class StampingBehavior extends BeltProcessingBehaviour {
    ...
    public StampingBehaviorSpecifics specifics;
    ...
    public interface StampingBehaviorSpecifics {
    ...
    public <T extends SmartBlockEntity & StampingBehaviorSpecifics> StampingBehavior(T be) {
        super(be);
        this.specifics = be;
        mode = Mode.WORLD;
        entityScanCooldown = ENTITY_SCAN;
        whenItemEnters((s, i) -> BeltStampingCallbacks.onItemReceived(s, i, this));
        whileItemHeld((s, i) -> BeltStampingCallbacks.whenItemHeld(s, i, this));
    }
    ...
    protected void spawnParticles() {
        ...
        if (mode == Mode.BELT)
            particleItems.forEach(stack -> makeStampingParticleEffect(VecHelper.getCenterOf(worldPosition.below(2))
                    .add(0, 8 / 16f, 0), stack));
        if (mode == Mode.WORLD)
            particleItems.forEach(stack -> makeStampingParticleEffect(VecHelper.getCenterOf(worldPosition.below(1))
                    .add(0, -1 / 4f, 0), stack));

        particleItems.clear();
    }

    public void makeStampingParticleEffect(Vec3 pos, ItemStack stack) {
        makeStampingParticleEffect(pos, stack, specifics.getParticleAmount());
    }

    public void makeStampingParticleEffect(Vec3 pos, ItemStack stack, int amount) {
        Level level = getWorld();
        if (level == null || !level.isClientSide)
            return;
